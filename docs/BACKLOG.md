...
- **Score**: 10
- **What**: Add TRX test logger + GitHub test reporter action. Inline test failure details in Actions — see which test failed, not just exit code 1.
- **Why**: 56 tests passing is meaningless if you can't see which one broke. Traceability at CI level.
- **Acceptance criteria**:
  - Failed test names visible directly in GitHub Actions summary
  - No change to test logic or domain code
  - Build still passes green
...
- **Acceptance criteria**:
  ...
  - [spec.md](http://spec.md) updated with trays profile section
...
- **Acceptance criteria**:
  ...
  - Version convention documented in [decisions.md](http://decisions.md)
...
#### [BACK-009] Fix ADR-003 gap in [decisions.md](http://decisions.md)
...
- **Why**: [decisions.md](http://decisions.md) goes 001 → 002 → 004. Either ADR-003 was deleted, renamed, or never written.
- **Acceptance criteria**:
...
| Documentation structure aligned    | 2026-05-07 | _[INDEX.md](http://INDEX.md), [ARCHITECTURE.md](http://ARCHITECTURE.md), single README |
| CI/CD pipeline green               | 2026-05-07 | dotnet.yml upgraded to @v5, Release config fixed                                       |
| Mermaid diagrams fixed             | 2026-05-07 | [spec.md](http://spec.md) diagram rendering on GitHub                                  |
